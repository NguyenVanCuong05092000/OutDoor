...
import brite.outdoor.databinding.FrmCameraBinding
import brite.outdoor.ui.fragments.BaseFragmentMedia
import brite.outdoor.R
import brite.outdoor.analyzer.LuminosityAnalyzer
...
class CameraFragment : BaseFragmentMedia<FrmCameraBinding>(R.layout.frm_camera) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        flashMode = PrefManager.getInstance(requireContext()).getInt(KEY_FLASH, FLASH_MODE_OFF)
        hasGrid = PrefManager.getInstance(requireContext()).getBoolean(KEY_GRID, false)
        hasHdr = PrefManager.getInstance(requireContext()).getBoolean(KEY_HDR, false)
        initViews()
    ...
    private fun closeFlashAndSelect(@FlashMode flash: Int) =
        binding.llFlashOptions.circularClose(binding.btnFlash) {
            ...
            PrefManager.getInstance(requireContext()).writeInt(KEY_FLASH, flashMode)
        }
    ...
    private fun toggleHdr() {
        binding.btnHdr.toggleButton(
            ...
            hasHdr = flag
            PrefManager.getInstance(requireContext()).writeBoolean(KEY_HDR, flag)
            startCamera()
        }
    }
